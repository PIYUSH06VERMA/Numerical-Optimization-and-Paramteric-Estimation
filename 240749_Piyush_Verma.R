...
df<-c(0.532,0.568,0.881,0.755,0.203,0.292,0.282,0.839,0.364,1.072,1.075,1.070,1.153,
      0.456,0.861,0.714,1.273,0.814,0.765,0.215,0.412,1.397,0.855,0.118,0.294,0.458,
      0.072,0.695 ) ## fort.58 dataset
...
{
  n<-length(df)
  out<-n*log(-n/(sum(log(1-exp(-l*df)))))+n*log(l)-l*sum(df)-sum(log(1-exp(-l*df)))-n
  return(out)
}
...
## Plotting the profile log-likelihood for lambda to visually verify the MLE
## Vectorizing g_lambda so curve() can evaluate it over a sequence of x values
g_lambda_vec <- Vectorize(g_lambda)

curve(g_lambda_vec(x), 
      from = max(0.1, lambda_hat - 5), 
      to = lambda_hat + 5, 
      col = "blue", 
      lwd = 2, 
      xlab = expression(lambda), 
      ylab = "Profile Log-Likelihood",
      main = "Profile Log-Likelihood vs. Lambda")

## Add a vertical line at the computed MLE
abline(v = lambda_hat, col = "red", lty = 2, lwd = 2)

## Add a point at the exact maximum
points(lambda_hat, g_lambda_vec(lambda_hat), col = "red", pch = 19, cex = 1.5)

legend("topright", 
       legend = c("Profile Log-Likelihood", sprintf("Computed MLE: %.4f", lambda_hat)),
       col = c("blue", "red"), 
       lty = c(1, 2), 
       lwd = c(2, 2), 
       pch = c(NA, 19))
